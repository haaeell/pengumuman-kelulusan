...
@section('content')
    <style>
        ...
        .stat-card.red    { background:#fff1f2; border:1px solid #fecdd3; }
        ...
        .stat-card.red .stat-icon    { background:#ffe4e6; color:#dc2626; }
        ...
        #studentsTable_wrapper .dataTables_length select:focus { border-color: #6366f1 !important; }
        ...
        #studentsTable tbody tr:hover td { background: #fafbff !important; }
        ...
        .badge-gagal  { background:#ffe4e6; color:#be123c; }
        ...
        .action-btn.edit { background:#fef9c3; color:#ca8a04; }
        .action-btn.edit:hover { background:#fde047; color:#854d0e; }
        .action-btn.del  { background:#ffe4e6; color:#e11d48; }
        .action-btn.del:hover  { background:#fecdd3; color:#9f1239; }
        ...
        .btn-primary-indigo:hover { transform: translateY(-1px); box-shadow: 0 4px 12px rgba(99,102,241,.45); }

        /* Mobile: button group stacks */
        @media (max-width: 640px) {
            .page-header { flex-direction: column; align-items: stretch !important; gap: 12px; }
            .page-header .btn-group { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; }
            .page-header .btn-group .btn-primary-indigo { grid-column: 1 / -1; justify-content: center; }
            .btn-outline-green, .btn-outline-red, .btn-primary-indigo { justify-content: center; }

            .stat-card { padding: 14px 16px; }
            .stat-card h3.text-3xl { font-size: 1.6rem; }

            #studentsTable_wrapper .dataTables_filter,
            #studentsTable_wrapper .dataTables_length { width: 100%; }
            #studentsTable_wrapper .dataTables_filter input { width: 100% !important; }
            #studentsTable_wrapper .dataTables_info,
            #studentsTable_wrapper .dataTables_paginate { font-size: 12px; }
        }
        ...
        .modal-close {
            ...
            display: flex; align-items: center; justify-content: center;
            color: #64748b; font-size: 13px; transition: all .15s;
            flex-shrink: 0;
        }
        ...
        .form-label  {
            ...
            font-size: 12px; font-weight: 600; color: #374151; letter-spacing: .02em;
        }
        .form-control {
        ...
        .mbtn-green {
            ...
            color: white; box-shadow: 0 2px 8px rgba(34,197,94,.3);
        }
        .mbtn-green:hover { transform: translateY(-1px); box-shadow: 0 4px 12px rgba(22,163,74,.4); }
        ...
        .import-step:hover { border-color: #c7d2fe; }
        .step-badge {
            display: inline-flex; align-items: center; justify-content: center;
        ...
        .upload-zone:hover { background: #eff6ff; border-color: #818cf8; }
        ...
        @media (max-width: 768px) {
            #studentsTable_wrapper .dataTables_filter { text-align: left; }
            #studentsTable_wrapper .row { margin: 0; }
        }
    </style>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <div class="stat-card green">
                ...
                <div>
                    ...
                    <h3 class="text-3xl font-800 text-green-700 leading-none">{{ $students->where('status','lulus')->count() }}</h3>
                </div>
            </div>
            <div class="stat-card red">
                ...
                <div>
                    ...
                    <h3 class="text-3xl font-800 text-red-700 leading-none">{{ $students->where('status','tidak_lulus')->count() }}</h3>
                </div>
            </div>
                    ...
                    <table class="w-full text-sm" id="studentsTable" style="min-width:700px;">
                        ...
                        <tbody>
                            @foreach ($students as $s)
                                <tr>
                                    <td class="text-gray-400 font-500">{{ $loop->iteration }}</td>
                                    <td><span class="font-mono text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-md">{{ $s->nis }}</span></td>
                                    <td class="font-600 text-gray-800">{{ $s->nama }}</td>
                                    <td><span class="bg-indigo-50 text-indigo-600 text-xs font-600 px-2 py-1 rounded-lg">{{ $s->kelas }}</span></td>
                                    <td class="font-600">{{ $s->total_score }}</td>
                                    <td>{{ number_format($s->average_score, 2) }}</td>
                                    <td>
                                        <span class="flex items-center gap-1">
                                            <span class="text-amber-500 text-xs"><i class="fa-solid fa-hashtag"></i></span>
                                            <span class="font-700">{{ $s->ranking }}</span>
                                        </span>
                                    </td>
                                    <td>
                                        @if ($s->status == 'lulus')
                                            <span class="badge badge-lulus"><i class="fa-solid fa-circle-check text-[9px]"></i> LULUS</span>
                                        @else
                                            <span class="badge badge-gagal"><i class="fa-solid fa-circle-xmark text-[9px]"></i> TIDAK LULUS</span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="flex items-center justify-center gap-1.5">
                                            <button onclick="openModal('edit{{ $s->id }}')" class="action-btn edit" title="Edit">
                                                <i class="fa-solid fa-pen-to-square"></i>
                                            </button>
                                            <button onclick="hapus({{ $s->id }})" class="action-btn del" title="Hapus">
                                                <i class="fa-solid fa-trash-can"></i>
                                            </button>
                                            <form id="hapus-{{ $s->id }}" method="POST" action="{{ route('students.destroy', $s->id) }}" class="hidden">
                                                @csrf @method('DELETE')
                                            </form>
                                        </div>
                                    </td>
            ...
            <div class="modal-body">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input name="total_score" class="form-control" type="number" step="0.01" placeholder="0.00" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Rata‑rata Nilai</label>
                        <input name="average_score" class="form-control" type="number" step="0.01" placeholder="0.00" required>
                    </div>
                </div>
                <div class="form-grid">
                    <div class="form-group">
        ...
        <form method="POST" action="{{ route('students.import') }}" enctype="multipart/form-data" class="modal-box wide">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" id="btnImport" class="mbtn mbtn-green">
                    <svg id="importSpinner" class="hidden w-4 h-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            ...
            <form method="POST" action="{{ route('students.update', $s->id) }}" class="modal-box">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            <label class="form-label">Total Nilai</label>
                            <input name="total_score" class="form-control" type="number" step="0.01" value="{{ $s->total_score }}">
                        </div>
                        <div class="form-group">
                            <label class="form-label">Rata‑rata Nilai</label>
                            <input name="average_score" class="form-control" type="number" step="0.01" value="{{ $s->average_score }}">
                        </div>
                    </div>
                    <div class="form-grid">
                        <div class="form-group">
